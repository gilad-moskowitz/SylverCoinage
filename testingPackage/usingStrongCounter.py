    ...
    def findWinningMove(self, gaps):
        ...
        for a in possibleMatches:
            if(len(possibleMatches[a]) == 0): 
                nextStage = self.pretendMove(gaps, a)
                if(self.checkStronglyLostPosition(nextStage)[0] and a > 1):
                    return a
        for b in possibleMatches:
            if(len(possibleMatches[b]) == 0): 
                nextStage = self.pretendMove(gaps, b)
                if(self.checkWeak(nextStage) and b > 1):
                    return b
        return max(gaps)
