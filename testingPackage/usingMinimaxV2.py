...
class minimaxBot:
    ...
    def newMethodWinningMove(self, gaps):
        split = int((len(gaps) + ( (-len(gaps))%3))/3)
        batch1 = [gaps[i] for i in range(0, split)]
        batch2 = [gaps[j] for j in range(split, min(2*split, len(gaps)))]
        batch3 = [gaps[k] for k in range(min(2*split, len(gaps)), min(3*split, len(gaps)))]
        for move in batch1:
            if(move > 1):
    ...
    def nextMove(self, movesPlayed, remainingGaps = []):
        ...
        if((len(remainingMoves) < 40) and (len(remainingMoves) > 0) and (len(movesPlayed) != 0)):
            return self.newMethodWinningMove(remainingMoves)
        #NORMAL STUFF
